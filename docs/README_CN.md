...
v1.19.1 是一个 **PATCH 热修复版本**，解决 **Gemini 摄入时 HTTP 400 错误（Issue #137）**。新增三层思考控制方言回退链（anthropic → openai → none），OpenAI 兼容客户端可自动发现每个 baseUrl 的正确思考控制字段名并缓存结果，后续请求跳过探测。设置页面关闭时不再擦除刚缓存的探测结果。通用的字段剥离重试（温度、重复惩罚等）和流式路径字段剥离修复完善了本次发布。Gemini 用户在测试连接后即可正常摄入。

- **🤖 Gemini HTTP 400 修复（Issue #137）。** 三层思考控制方言回退链（anthropic → openai → none）在测试连接时探测正确字段名并缓存。设置面板中「高级参数设置」移至「测试连接」上方以获得更好的操作流程。
- **🛡️ 通用 400 字段剥离重试。** `unsupportedFields` Set 从错误体中提取被拒绝的字段名，后续请求自动预剥离。同时在流请求路径上生效（之前是死代码）。
- **🔊 降级通知现在支持本地化。** `queueFallbackNotice()` 尊重用户语言设置——3 个 i18n 键（`fallbackThinkingDialect`、`fallbackThinkingNone`、`fallbackParamStripped`）现在对全部 7 种非英文语言生效。
- **🧹 多项代码简化。** `IS_400` 正则提取为模块级常量、`retryBodyWithStrippedFields` 辅助方法抽取、`commitTempSettings()` 去重设置合并逻辑、`applyThinkingDialectFallback` 复用 `buildRequestBody`（修复了潜藏的 unsupportedFields 预剥离泄漏）。
- **📊 控制台诊断噪音减少。** `[OpenAICompat Debug] 400 body` 从 `console.error` 降级到 `console.debug`；`[DEBUG-400]` 重抓取限定在 400 级错误（之前错误地在 429 配额错误上触发）。
