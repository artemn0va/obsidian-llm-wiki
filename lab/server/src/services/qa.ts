...
function unwrapBrokenWikiLinks(content: string, existingTargets: Set<string>): { content: string; count: number } {
  const linkRegex = /\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]/g;
  let count = 0;
  const nextContent = content.replace(linkRegex, (match, rawTarget: string, label?: string) => {
...
function fixLegacySourceFileFrontmatter(content: string): { content: string; count: number } {
  ...
  if (/^source_path:/m.test(frontmatter)) {
    ...
    nextFrontmatter = frontmatter.replace(/^source_file:/gm, () => {
      count += 1;
      return 'source_path:';
    });
  }
